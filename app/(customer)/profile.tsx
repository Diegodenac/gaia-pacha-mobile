...
function PasswordInput({
  value, onChange, placeholder, error, textContentType = 'password',
}: {
  value: string;
...
function AuthModal({
  ...
  const {
    control: lCtrl,
    handleSubmit: lSubmit,
    formState: { errors: lErr },
    reset: lReset,
  } = useForm<LoginForm>({
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <PasswordInput
                        ...
                        error={sErr.confirmPassword?.message}
                        textContentType="newPassword"
                      />
                    )}
                  />
...
export default function CustomerProfileScreen() {
        ...
        >
          ...
          <View style={p.userCard}>
            ...
            <View style={p.userInfo}>
              <Text style={p.userName}>{user.email}</Text>
  ...
  return (
    <SafeAreaView style={p.root} edges={['top']}>
      <ScrollView
        contentContainerStyle={p.guestScroll}
        showsVerticalScrollIndicator={false}
      >
        {/* Hero */}
        <View style={p.hero}>
...
const m = StyleSheet.create({
  ...
  sheetTitle:    { fontSize: 20, fontWeight: '700', color: TEXT_PRIMARY },
  closeBtn:      { padding: 4 },
  formBody:      { paddingBottom: 8 },
  fieldWrap:     { marginBottom: 14 },
  label:         { fontSize: 13, fontWeight: '600', color: TEXT_SECONDARY, marginBottom: 6 },
  input:         { backgroundColor: SURFACE_INPUT, borderWidth: 1, borderColor: BORDER, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, fontSize: 16, color: TEXT_PRIMARY },
  ...
  pwdInput:      { flex: 1, paddingHorizontal: 16, paddingVertical: 12, fontSize: 16, color: TEXT_PRIMARY },
  ...
  submitBtnText: { color: TEXT_PRIMARY, fontSize: 16, fontWeight: '700' },
...
const p = StyleSheet.create({
  ...
  screenTitle:   { fontSize: 22, fontWeight: '800', color: TEXT_PRIMARY, marginBottom: 20 },
  ...
  avatarCircle:  { width: 56, height: 56, borderRadius: 28, backgroundColor: '#14532d', borderWidth: 2, borderColor: PRIMARY, alignItems: 'center', justifyContent: 'center' },
  avatarText:    { fontSize: 20, fontWeight: '800', color: PRIMARY_LIGHT },
  userInfo:      { flex: 1, gap: 2 },
  userName:      { fontSize: 15, fontWeight: '600', color: TEXT_PRIMARY },
  ...
  heroTitle:      { fontSize: 30, fontWeight: '800', color: TEXT_PRIMARY, marginBottom: 10, letterSpacing: -0.5 },
  ...
  ctaPrimaryText: { fontSize: 16, fontWeight: '700', color: TEXT_PRIMARY },
